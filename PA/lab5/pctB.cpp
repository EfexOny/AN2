...
void addLista(lista *&curr,arbore *toInsert){
    ...
    if(curr==nullptr || toInsert->data < curr->arb->data ){
        ...
        // crescator
        while(parc->next && parc->next->arb->data < toInsert->data && parc->next){
            parc=parc->next;
        }
        nou->next=parc->next;
        parc->next=nou;
...
arbore* removeLowest(lista *&list){
    ...
    // return la primu elem din lista mereu ca e ordonata
    arbore *ret=nullptr;
    if(list){
        ret=list->arb;
        list=list->next;
